...
public class OrderServiceImpl implements IOrderService{
    ...
    public ServerResponse create(Integer userId, Integer shippingId) {
        //参数非空校验
        if(shippingId==null){
            return ServerResponse.createServerResponseByFail(9,"地址参数不能为空");

        }

        //根据userId查询购物车中已选商品List<Cart>
        ...
        if(orderItemList==null||orderItemList.size()==0){
            return ServerResponse.createServerResponseByFail(100,"购物车为空");
        }
        orderTotalPrice=getOrderPrice(orderItemList);
        Order order=createOrder(userId,shippingId,orderTotalPrice);
        if(order==null){
            return ServerResponse.createServerResponseByFail(1,"创建订单失败");
        }
    ...
    public ServerResponse cancel(Integer userId, Long orderNo) {
        ...
        if(order==null){
            return ServerResponse.createServerResponseByFail(1,"该用户没有此订单");
        }
        //判断订单状态并取消
    ...
    public ServerResponse get_order_cart_product(Integer userId) {
        ...
        if(orderItemList==null||orderItemList.size()==0){
            return ServerResponse.createServerResponseByFail(1,"购物车为空");
        }
        for(OrderItem orderItem:orderItemList){
    ...
    * 创建订单
    * */
    private Order createOrder(Integer userId, Integer shippingId, BigDecimal orderTotalPrice)throws MyException{
    ...
    * */
    ...
    private ServerResponse getCartOrderItem(Integer userId,List<Cart> cartList){
         ...
         for(Cart cart:cartList){
             ...
             if(product==null){
                 return ServerResponse.createServerResponseByFail(4,"id为"+cart.getProductId()+"的商品不存在");
             }
             if(product.getStatus()!= Const.ProductStatus.PRODUCT_ONLINE.getCode()){
                 return ServerResponse.createServerResponseByFail(4,"id为"+product.getId()+"的商品已下架");
             }
             if(product.getStock()<cart.getQuantity()){
                 return ServerResponse.createServerResponseByFail(3,"id为"+product.getId()+"的商品库存不足");
             }
    ...
    * */
    ...
    public ServerResponse alipay_callback(Map<String, String> map) {
        ...
        Order order=orderMapper.findOrderByOrderNo(orderNo);
        if(order==null){
            return ServerResponse.createServerResponseByFail(100,"订单"+orderNo+"不是本商品的订单");
        }
        if(order.getStatus()>=Const.OrderStatusEnum.ORDER_PAYED.getCode()){
            return ServerResponse.createServerResponseByFail(100,"支付宝重复调用");
        }
        ...
        return ServerResponse.createServerResponseByFail(100,"支付信息保存失败");
    ...
    public ServerResponse query_order_pay_status(Integer userId,Long orderNo) {
        //参数校验
        if(orderNo==null){
            return ServerResponse.createServerResponseByFail(100,"订单号不能为空");
        }
        Order order=orderMapper.findOrderByUserIdAndOrderNo(userId,orderNo);
        if(order==null){
            return ServerResponse.createServerResponseByFail(1,"该用户并没有该订单,查询无效");
        }
        if(order.getStatus()==Const.OrderStatusEnum.ORDER_PAYED.getCode()){
    ...
    public ServerResponse pay(Order order) throws IOException {
        ...
        switch (result.getTradeStatus()) {
            case SUCCESS:
                ...
                // 需要修改为运行机器上的路径
                String filePath = String.format(filepath,
                        response.getOutTradeNo());
                //D:/upload/qr/qr-%s.png
                log.info("filePath:" + filePath);
                ZxingUtils.getQRCodeImge(response.getQrCode(), 256, filePath);
                File file=new File(filePath);
                /*FTPUtil.uploadFile(Lists.newArrayList(file));*/
                String img=uploadService.uploadFile(file);
                Map map= Maps.newHashMap();
                map.put("order",order.getOrderNo());
                map.put("qrCode",imageHost+"/"+img);
